Public Class FormScheduler
        ...
        Try
            ...
            If get_opt_scheduler_field("is_active_line_list").ToString = "1" Then
                If Date.Parse(TELineList.EditValue.ToString).ToString("HH:mm:ss") = cur_datetime.ToString("HH:mm:ss") Then
                    Dim check_line_list_date As String = cur_datetime.ToString("yyyy-MM-dd")
                    Dim ll As New ClassLineList()
                    Dim dtl As DataTable = ll.dataNotifSummary(check_line_list_date)
                    If dtl.Rows.Count > 0 Then
                        'line list notif
                        Try
                            Dim sm As New ClassSendEmail()
                            sm.report_mark_type = "397"
                            sm.par1 = check_line_list_date
                            sm.dt = dtl
                            sm.send_email_line_list()
                        Catch ex As Exception
                            Dim qlog As String = "INSERT INTO tb_log_line_list_mail(log_note, log_date, id_line_list_notif_type, check_line_list_date) VALUES('Error Notif Line List:" + addSlashes(ex.ToString) + "', NOW(),1, '" + check_line_list_date + "'); "
                            execute_non_query(qlog, True, "", "", "", "")
                        End Try

                        'drop/changes
                        Try
                            Dim qcek As String = "CALL view_drop_changes_list_v2('" + check_line_list_date + "')"
                            ...
                            End If
                        Catch ex As Exception
                            Dim qlog As String = "INSERT INTO tb_log_line_list_mail(log_note, log_date, id_line_list_notif_type, check_line_list_date) VALUES('Error Notif Drop/Changes:" + addSlashes(ex.ToString) + "', NOW(),2, '" + check_line_list_date + "'); "
                            execute_non_query(qlog, True, "", "", "", "")
                        End Try

                        'eta update
                        Try
                            Dim qeta As String = "CALL view_eta_changes_list_v2('" + check_line_list_date + "')"
                            ...
                            End If
                        Catch ex As Exception
                            Dim qlog As String = "INSERT INTO tb_log_line_list_mail(log_note, log_date, id_line_list_notif_type, check_line_list_date) VALUES('Error Notif ETA Update:" + addSlashes(ex.ToString) + "', NOW(),3, '" + check_line_list_date + "'); "
                            execute_non_query(qlog, True, "", "", "", "")
                        End Try
                    End If
                End If
    ...
    Private Sub BtnResendEmail_Click(sender As Object, e As EventArgs) Handles BtnResendEmail.Click
        Cursor = Cursors.WaitCursor
        FormResendNotifLineList.ShowDialog()
        Cursor = Cursors.Default
    End Sub
End Class